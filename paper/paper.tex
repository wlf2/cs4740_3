...
into the scorer.  \verb+classify.py+ includes a help switch \verb+-h+, that
explains the switches that control the different classifiers and features along
with any additional parameters that a certain feature might support (for
example, collocation has a window size parameter which defaults to 0)
Performance was excellent for most of the features, allowing us to typically run
all of the words in the test set through every combination a classifier and
feature set in under an hour, although dependency parsing was slow (The number
of runs can get quite high when you factor in several parameters per feature,
not just on/off)
...
the co-occurrence feature extractor with the \naive{} Bayes classifier
...
Second, we plotted histograms of sentence length. The distributions appeared to be
...
returned on either side of the word.

This data would then be converted into
...
uninformative.  There were very few instances of the words that we were
looking for (e.g. activate only appeared 3 times), which made searching through
